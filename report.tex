...
Since DBSCAN produced only one cluster, the feature space was modified using PCA reduction:

\begin{table}[H]
\centering
\caption{DBSCAN with PCA-Reduced Space}
\begin{tabular}{lc}
\toprule
\textbf{Metric} & \textbf{Value} \\
\midrule
...
New Epsilon & 1.85 \\
Clusters detected & 1 \\
Noise points & 18 (10.1\%) \\
\bottomrule
\end{tabular}
\end{table}

The Wine dataset's distribution does not form clearly separated density regions suitable for DBSCAN with standard parameter estimation approaches.
